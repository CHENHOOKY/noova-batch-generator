...
from PySide6.QtWidgets import (
    ...
    QProgressBar, QComboBox, QSpinBox, QCheckBox, QLineEdit,
...
# ═══════════════════════════════════════════════════════
#  文本 API 客户端（纯 stdlib，零依赖）
# ═══════════════════════════════════════════════════════

def _call_text_api(api_key: str, base_url: str, model: str,
                   system_prompt: str, user_prompt: str,
                   temperature: float = 0.7, max_tokens: int = 16384,
                   json_mode: bool = True, log_fn=None) -> str:
    ...
    try:
        ...
        if e.code == 401:
            raise RuntimeError("文本 API Key 无效 (401)，请检查后重试")
        if e.code == 429:
            raise RuntimeError(f"文本 API 请求过于频繁 (429)，请稍后重试")
        raise RuntimeError(f"文本 API HTTP {e.code}: {body_text[:300]}")
    except urllib.error.URLError as e:
        raise RuntimeError(f"文本 API 连接失败: {e.reason}")
    except (json.JSONDecodeError, KeyError, IndexError, TypeError) as e:
        raise RuntimeError(f"文本 API 响应格式异常: {e}") from e


def _call_text_api_with_retry(api_key: str, base_url: str, model: str,
                               system_prompt: str, user_prompt: str,
        ...
        try:
            ...
            if "TRUNCATED" in msg:
                ...
                if attempt < max_retries - 1:
                    wait = (2 ** attempt) * 3
                    if log_fn:
                        log_fn(f"  [RETRY] {msg[:80]}，{wait}s 后重试...")
                    time.sleep(wait)
    raise last_error


def _extract_json(text: str) -> dict:
    """从文本中提取 JSON 对象（处理 markdown 代码块）"""
    text = text.strip()
    # 去除 markdown 代码块包裹
